...
| Outbound mode | `/runtime/mode` is a runtime override in front of the routing table: `direct` short-circuits to the direct outbound, `global` to one chosen outbound, both after the engine's must presets; dropped on activation. | New engine state; the routing program consults it before rule evaluation. |
